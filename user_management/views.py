...
# from AIRSENSE.settings import EMAIL_HOST_USER
...
def login(request):
    if 'logged_in' in request.session:
        if request.session['logged_in'] is True:
            ...
            # return redirect("airsense_app:home")
    else:
...
def login_validate(request):
    if request.method == 'POST':
        ...
        if user is None:
            ...
            if user.password == password_enc and user.username == username:
                if user.is_active == 1:
                    try:
...
def user_registration(request):
    ...
    if request.method == 'GET':
        return render(request, 'user_management/user_registration.html', context)
    if request.method == 'POST':
        username = request.POST['username']
